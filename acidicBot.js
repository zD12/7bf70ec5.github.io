    ...
    var acidicBot = {
        ...
        room: {
            ...
            roulette: {
                ...
                endRoulette: function () {
                    ...
                    API.sendChat(subChat(acidicBot.chat.winnerpicked, {name: name, position: pos}));
        ...
        userUtilities: {
                ...
                if (API.getWaitListPosition(id) === -1) {
                    if (wlist.length < 50) {
                        ...
                        if (alreadyQueued !== -1) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.alreadyadding, {position: acidicBot.room.queue.position[alreadyQueued]}));
                        }
                        ...
                        var name = user.username;
                        return API.sendChat(subChat(acidicBot.chat.adding, {name: name, position: acidicBot.room.queue.position.length}));
                    }
                }
            ...
            dclookup: function (id) {
                ...
                var name = user.username;
                if (user.lastDC.time === null) return subChat(acidicBot.chat.notdisconnected, {name: name});
                ...
                if (!validDC) return (subChat(acidicBot.chat.toolongago, {name: acidicBot.userUtilities.getUser(user).username, time: time}));
                ...
                var msg = subChat(acidicBot.chat.valid, {name: acidicBot.userUtilities.getUser(user).username, time: time, position: newPosition});
                        ...
                        if (typeof user !== 'boolean') {
                            ...
                            if (rank !== null && acidicBot.userUtilities.getPermission(plugUser) <= rank) {
                                ...
                                if (inactivity > acidicBot.settings.maximumAfk * 60 * 1000) {
                                    if (warncount === 0) {
                                        API.sendChat(subChat(acidicBot.chat.warning1, {name: name, time: time}));
                                        ...
                                        API.sendChat(subChat(acidicBot.chat.warning2, {name: name}));
                                        user.afkWarningCount = 3;
                                        user.afkCountdown = setTimeout(function (userToChange) {
                                        ...
                                        if (pos !== -1) {
                                            ...
                                            API.sendChat(subChat(acidicBot.chat.afkremove, {name: name, time: time, position: pos, maximumafk: acidicBot.settings.maximumAfk}));
                                        }
                                        user.afkWarningCount = 0;
                                    }
                                }
        ...
        eventChat: function (chat) {
            ...
            if (!acidicBot.chatUtilities.commandCheck(chat))
                acidicBot.chatUtilities.action(chat);
        },
            ...
            if (acidicBot.settings.welcome && greet) {
                welcomeback ?
                    setTimeout(function (user) {
                        API.sendChat(subChat(acidicBot.chat.welcomeback, {name: user.username}));
                    }, 1 * 1000, user)
                    :
                    setTimeout(function (user) {
                        API.sendChat(subChat(acidicBot.chat.welcome, {name: user.username}));
                    }, 1 * 1000, user);
            }
        },
        ...
        eventVoteupdate: function (obj) {
            ...
            if (acidicBot.settings.voteSkip) {
                if ((mehs - woots) >= (acidicBot.settings.voteSkipLimit)) {
                    API.sendChat(subChat(acidicBot.chat.voteskipexceededlimit, {name: dj.username, limit: acidicBot.settings.voteSkipLimit}));
        ...
        eventDjadvance: function (obj) {
            ...
            if (acidicBot.settings.songstats) {
                if (typeof acidicBot.chat.songstatistics === "undefined") {
                    ...
                    API.sendChat(subChat(acidicBot.chat.songstatistics, {artist: lastplay.media.author, title: lastplay.media.title, woots: lastplay.score.positive, grabs: lastplay.score.grabs, mehs: lastplay.score.negative}))
                }
            }
            ...
            for (var bl in acidicBot.room.blacklists) {
                if (acidicBot.settings.blacklistEnabled) {
                    if (acidicBot.room.blacklists[bl].indexOf(mid) > -1) {
                        API.sendChat(subChat(acidicBot.chat.isblacklisted, {blacklist: bl}));
            ...
            if (acidicBot.settings.timeGuard && newMedia.duration > acidicBot.settings.maximumSongLength * 60 && !acidicBot.room.roomevent) {
                ...
                API.sendChat(subChat(acidicBot.chat.timelimit, {name: name, maxlength: acidicBot.settings.maximumSongLength}));
                return API.moderateForceSkip();
            }
                ...
                var historySkip = setTimeout(function () {
                    for (var i = 0; i < apihistory.length; i++) {
                        if (apihistory[i].media.cid === obj.media.cid) {
                            ...
                            alreadyPlayed = true;
                            API.sendChat(subChat(acidicBot.chat.songknown, {name: name}));
                            return API.moderateForceSkip();
                        }
                    }
                    if (!alreadyPlayed) {
            ...
            if (user.ownSong) {
                API.sendChat(subChat(acidicBot.chat.permissionownsong, {name: user.username}));
                user.ownSong = false;
            }
        ...
        chatcleaner: function (chat) {
            ...
            if (capitals >= 40) {
                API.sendChat(subChat(acidicBot.chat.caps, {name: chat.un}));
                return true;
            }
            msg = msg.toLowerCase();
            if (msg === 'skip') {
                API.sendChat(subChat(acidicBot.chat.askskip, {name: chat.un}));
                return true;
            }
            for (var j = 0; j < acidicBot.chatUtilities.spam.length; j++) {
                if (msg === acidicBot.chatUtilities.spam[j]) {
                    API.sendChat(subChat(acidicBot.chat.spam, {name: chat.un}));
                    return true;
                }
            }
            return false;
        ...
        chatUtilities: {
            chatFilter: function (chat) {
                ...
                if (msg.indexOf('http://adf.ly/') > -1) {
                    API.moderateDeleteChat(chat.cid);
                    API.sendChat(subChat(acidicBot.chat.adfly, {name: chat.un}));
                    return true;
                }
        ...
        startup: function () {
            ...
            loadChat(API.sendChat(subChat(acidicBot.chat.online, {botname: acidicBot.settings.botName, version: acidicBot.version})));
        },
        commands: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length === cmd.length) time = 60;
                            ...
                            if (isNaN(time)) return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                        }
                        for (var i = 0; i < acidicBot.room.users.length; i++) {
                        ...
                        API.sendChat(subChat(acidicBot.chat.activeusersintime, {name: chat.un, amount: chatters, time: time}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length > cmd.length + 2) {
                            ...
                            } else API.sendChat(subChat(acidicBot.chat.invaliduserspecified, {name: chat.un}));
                        }
                    }
                }
            },
            ...
            afklimitCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat(subChat(acidicBot.chat.nolimitspecified, {name: chat.un}));
                        var limit = msg.substring(cmd.length + 1);
                        if (!isNaN(limit)) {
                            acidicBot.settings.maximumAfk = parseInt(limit, 10);
                            API.sendChat(subChat(acidicBot.chat.maximumafktimeset, {name: chat.un, time: acidicBot.settings.maximumAfk}));
                        }
                        else API.sendChat(subChat(acidicBot.chat.invalidlimitspecified, {name: chat.un}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.afkRemoval) {
                            ...
                            clearInterval(acidicBot.room.afkInterval);
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.afkremoval}));
                        }
                        else {
                            acidicBot.settings.afkRemoval = !acidicBot.settings.afkRemoval;
                            acidicBot.room.afkInterval = setInterval(function () {
                            ...
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.afkremoval}));
                        }
                    }
                }
            },
            ...
            afkresetCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.afkstatusreset, {name: chat.un, username: name}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (inactivity == durationOnline){
                            API.sendChat(subChat(acidicBot.chat.inactivelonger, {botname: acidicBot.settings.botName, name: chat.un, username: name}));
                        } else {
                        API.sendChat(subChat(acidicBot.chat.inactivefor, {name: chat.un, username: name, time: time}));
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.autodisable) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.autodisable}));
                        }
                        else {
                            acidicBot.settings.autodisable = !acidicBot.settings.autodisable;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.autodisable}));
                        }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.room.autoskip) {
                            ...
                            clearTimeout(acidicBot.room.autoskipTimer);
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.autoskip}));
                        }
                        else {
                            acidicBot.room.autoskip = !acidicBot.room.autoskip;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.autoskip}));
                        }
                    }
                }
            },
            ...
            ballCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                            ...
                            API.sendChat(subChat(acidicBot.chat.ball, {name: chat.un, botname: acidicBot.settings.botName, question: argument, response: acidicBot.chat.balls[randomBall]}));
                     }
                }
            },
            ...
            blacklistCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat(subChat(acidicBot.chat.nolistspecified, {name: chat.un}));
                        var list = msg.substr(cmd.length + 1);
                        if (typeof acidicBot.room.blacklists[list] === 'undefined') return API.sendChat(subChat(acidicBot.chat.invalidlistspecified, {name: chat.un}));
                        else {
                            ...
                            API.sendChat(subChat(acidicBot.chat.newblacklisted, {name: chat.un, blacklist: list, author: media.author, title: media.title, mid: media.format + ':' + media.cid}));
                            API.moderateForceSkip();
            ...
            blinfoCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.blinfo, {name: name, author: author, title: title, songid: songid}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.bouncerPlus) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': 'Bouncer+'}));
                        }
                        else {
                            if (!acidicBot.settings.bouncerPlus) {
                                var id = chat.uid;
                                ...
                                if (perm > 2) {
                                    ...
                                    return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': 'Bouncer+'}));
                                }
                            }
                            else return API.sendChat(subChat(acidicBot.chat.bouncerplusrank, {name: chat.un}));
                        }
                    }
                }
            },
            ...
            botnameCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length <= cmd.length + 1) return API.sendChat(subChat(acidicBot.chat.currentbotname, {botname: acidicBot.settings.botName}));
                        var argument = msg.substring(cmd.length + 1);
                        if (argument) {
                            acidicBot.settings.botName = argument;
                            API.sendChat(subChat(acidicBot.chat.botnameset, {botName: acidicBot.settings.botName}));
                        }
                    }
                }
            },
            ...
            clearchatCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        return API.sendChat(subChat(acidicBot.chat.chatcleared, {name: chat.un}));
                    }
                }
            },
            ...
            commandsCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.commandslink, {botname: acidicBot.settings.botName, link: acidicBot.cmdLink}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.cmdDeletion) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.cmddeletion}));
                        }
                        else {
                            acidicBot.settings.cmdDeletion = !acidicBot.settings.cmdDeletion;
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.cmddeletion}));
                        }
                    }
                }
            },
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (space === -1) {
                            ...
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(acidicBot.chat.nousercookie, {name: name}));
                            }
                            else if (user.username === chat.un) {
                                return API.sendChat(subChat(acidicBot.chat.selfcookie, {name: name}));
                            }
                            else {
                                return API.sendChat(subChat(acidicBot.chat.cookie, {nameto: user.username, namefrom: chat.un, cookie: this.getCookie()}));
                            }
                        }
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.cycleGuard) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.cycleguard}));
                        }
                        else {
                            acidicBot.settings.cycleGuard = !acidicBot.settings.cycleGuard;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.cycleguard}));
                        }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(cycleTime) && cycleTime !== "") {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.cycleguardtime, {name: chat.un, time: acidicBot.settings.maximumCycletime}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length === cmd.length) name = chat.un;
                            ...
                            var perm = acidicBot.userUtilities.getPermission(chat.uid);
                            if (perm < 2) return API.sendChat(subChat(acidicBot.chat.dclookuprank, {name: chat.un}));
                        }
                        var user = acidicBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat(subChat(acidicBot.chat.invaliduserspecified, {name: chat.un}));
            ...
            emojiCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.emojilist, {link: link}));
                    }
                }
            },
            ...
            etaCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (pos < 0) return API.sendChat(subChat(acidicBot.chat.notinwaitlist, {name: name}));
                        var timeRemaining = API.getTimeRemaining();
                        var estimateMS = ((pos + 1) * 4 * 60 + timeRemaining) * 1000;
                        var estimateString = acidicBot.roomUtilities.msToStr(estimateMS);
                        API.sendChat(subChat(acidicBot.chat.eta, {name: name, time: estimateString}));
                    }
                }
            },
            ...
            fbCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.fbLink === "string")
                            API.sendChat(subChat(acidicBot.chat.facebook, {link: acidicBot.settings.fbLink}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.filterChat) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.chatfilter}));
                        }
                        else {
                            acidicBot.settings.filterChat = !acidicBot.settings.filterChat;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.chatfilter}));
                        }
                    }
                }
            },
            ...
            ghostbusterCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = acidicBot.userUtilities.lookupUserName(name);
                        if (user === false || !user.inRoom) {
                            return API.sendChat(subChat(acidicBot.chat.ghosting, {name1: chat.un, name2: name}));
                        }
                        else API.sendChat(subChat(acidicBot.chat.notghosting, {name1: chat.un, name2: name}));     
                    }
                }
            },
                        ...
                        if (msg.length !== cmd.length) {
                            ...
                            get_id(api_key, tag, function(id) {
                                if (typeof id !== 'undefined') {
                                    API.sendChat(subChat(acidicBot.chat.validgiftags, {name: chat.un, id: id, tags: commatag}));
                                } else {
                                    API.sendChat(subChat(acidicBot.chat.invalidgiftags, {name: chat.un, tags: commatag}));
                                }
                            });
                        }
                        else {
                            ...
                            get_random_id(api_key, function(id) {
                                if (typeof id !== 'undefined') {
                                    API.sendChat(subChat(acidicBot.chat.validgifrandom, {name: chat.un, id: id}));
                                } else {
                                    API.sendChat(subChat(acidicBot.chat.invalidgifrandom, {name: chat.un}));
                                }
                            });
                        }
                    }
            ...
            helpCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.starterhelp, {link: link}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.historySkip) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.historyskip}));
                        }
                        else {
                            acidicBot.settings.historySkip = !acidicBot.settings.historySkip;
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.historyskip}));
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.room.roulette.rouletteStatus && acidicBot.room.roulette.participants.indexOf(chat.uid) < 0) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.roulettejoin, {name: chat.un}));
                        }
                    }
                }
            },
            ...
            jointimeCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.jointime, {namefrom: chat.un, username: name, time: timeString}));
                    }
                }
            },
            ...
            kickCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = acidicBot.userUtilities.lookupUserName(name);
                        var from = chat.un;
                        if (typeof user === 'boolean') return API.sendChat(subChat(acidicBot.chat.nouserspecified, {name: chat.un}));
                        ...
                        if (permFrom <= permTokick)
                            return API.sendChat(subChat(acidicBot.chat.kickrank, {name: chat.un}));

                        if (!isNaN(time)) {
                            API.sendChat(subChat(acidicBot.chat.kick, {name: chat.un, username: name, time: time}));
                        ...
                        else API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                    }
                }
            },
            ...
            languageCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length <= cmd.length + 1) return API.sendChat(subChat(acidicBot.chat.currentlang, {language: acidicBot.settings.language}));
                        var argument = msg.substring(cmd.length + 1);
                        ...
                        $.get("https://rawgit.com/Yemasthui/acidicBot/master/lang/langIndex.json", function (json) {
                            ...
                            if (typeof link === "undefined") {
                                API.sendChat(subChat(acidicBot.chat.langerror, {link: "http://git.io/vJ9nI"}));
                            }
                            else {
                                acidicBot.settings.language = argument;
                                loadChat();
                                API.sendChat(subChat(acidicBot.chat.langset, {language: acidicBot.settings.language}));
                            }
                        });
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (ind > -1) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.rouletteleave, {name: chat.un}));
                        }
                    }
                }
            },
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (perm >= 1 || isDj) {
                            if (media.format === 1) {
                                ...
                                API.sendChat(subChat(acidicBot.chat.songlink, {name: from, link: linkToSong}));
                            }
                            if (media.format === 2) {
                                SC.get('/tracks/' + media.cid, function (sound) {
                                    API.sendChat(subChat(acidicBot.chat.songlink, {name: from, link: sound.permalink_url}));
                                });
                            }
                        }
                    }
            ...
            lockdownCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.lockdownEnabled) {
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.lockdown}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.lockdown}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.lockGuard) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.lockguard}));
                        }
                        else {
                            acidicBot.settings.lockGuard = !acidicBot.settings.lockGuard;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.lockguard}));
                        }
                    }
                }
            },
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.room.skippable) {
                            ...
                            if (chat.message.length === cmd.length) {
                                API.sendChat(subChat(acidicBot.chat.usedlockskip, {name: chat.un}));
                                acidicBot.roomUtilities.booth.lockBooth();
                                setTimeout(function (id) {
                                    API.moderateForceSkip();
            ...
            lockskipposCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(pos)) {
                            acidicBot.settings.lockskipPosition = pos;
                            return API.sendChat(subChat(acidicBot.chat.lockskippos, {name: chat.un, position: acidicBot.settings.lockskipPosition}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.invalidpositionspecified, {name: chat.un}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(lockTime) && lockTime !== "") {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.lockguardtime, {name: chat.un, time: acidicBot.settings.maximumLocktime}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                    }
                }
            ...
            logoutCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.logout, {name: chat.un, botname: acidicBot.settings.botName}));
                        setTimeout(function () {
                            $(".logout").mousedown()
                        }, 1000);
                    }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(maxTime)) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.maxlengthtime, {name: chat.un, time: acidicBot.settings.maximumSongLength}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (isNaN(argument)) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.motdset, {msg: acidicBot.settings.motd}));
                        }
                        else {
                            acidicBot.settings.motdInterval = argument;
                            API.sendChat(subChat(acidicBot.chat.motdintervalset, {interval: acidicBot.settings.motdInterval}));
                        }
                    }
                }
            },
            ...
            moveCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = acidicBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat(subChat(acidicBot.chat.invaliduserspecified, {name: chat.un}));
                        if (user.id === acidicBot.loggedInID) return API.sendChat(subChat(acidicBot.chat.addbotwaitlist, {name: chat.un}));
                        if (!isNaN(pos)) {
                            API.sendChat(subChat(acidicBot.chat.move, {name: chat.un}));
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (lastSpace === msg.indexOf(' ')) {
                            ...
                            if (isNaN(time) || time == "" || time == null || typeof time == "undefined") {
                                return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                            }
                            name = msg.substring(cmd.length + 2, lastSpace);
                        }
                        var from = chat.un;
                        ...
                        if (permFrom > permUser) {
                            if (time > 45) {
                                API.sendChat(subChat(acidicBot.chat.mutedmaxtime, {name: chat.un, time: "45"}));
                        ...
                        else API.sendChat(subChat(acidicBot.chat.muterank, {name: chat.un}));
                    }
                }
            },
            ...
            opCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.opLink === "string")
                            return API.sendChat(subChat(acidicBot.chat.oplist, {link: acidicBot.settings.opLink}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length > cmd.length + 2) {
                            ...
                            } else API.sendChat(subChat(acidicBot.chat.removenotinwl, {name: chat.un, username: name}));
                        } else API.sendChat(subChat(acidicBot.chat.nouserspecified, {name: chat.un}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.etaRestriction) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.etarestriction}));
                        }
                        else {
                            acidicBot.settings.etaRestriction = !acidicBot.settings.etaRestriction;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.etarestriction}));
                        }
                    }
                }
            },
            ...
            rulesCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.rulesLink === "string")
                            return API.sendChat(subChat(acidicBot.chat.roomrules, {link: acidicBot.settings.rulesLink}));
                    }
                }
            },
            ...
            sessionstatsCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.sessionstats, {name: from, woots: woots, mehs: mehs, grabs: grabs}));
                    }
                }
            },
            ...
            skipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat(subChat(acidicBot.chat.skip, {name: chat.un}));
                        API.moderateForceSkip();
                        acidicBot.room.skippable = false;
                        setTimeout(function () {
                            acidicBot.room.skippable = true
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.songstats) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.songstats}));
                        }
                        else {
                            acidicBot.settings.songstats = !acidicBot.settings.songstats;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.songstats}));
                        }
                    }
                }
            },
            ...
            statusCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        msg += subChat(acidicBot.chat.activefor, {time: since});
            ...
            swapCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof user1 === 'boolean' || typeof user2 === 'boolean') return API.sendChat(subChat(acidicBot.chat.swapinvalid, {name: chat.un}));
                        if (user1.id === acidicBot.loggedInID || user2.id === acidicBot.loggedInID) return API.sendChat(subChat(acidicBot.chat.addbottowaitlist, {name: chat.un}));
                        var p1 = API.getWaitListPosition(user1.id) + 1;
                        var p2 = API.getWaitListPosition(user2.id) + 1;
                        if (p1 < 0 || p2 < 0) return API.sendChat(subChat(acidicBot.chat.swapwlonly, {name: chat.un}));
                        API.sendChat(subChat(acidicBot.chat.swapping, {'name1': name1, 'name2': name2}));
                        if (p1 < p2) {
            ...
            themeCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.themeLink === "string")
                            API.sendChat(subChat(acidicBot.chat.genres, {link: acidicBot.settings.themeLink}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.timeGuard) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.timeguard}));
                        }
                        else {
                            acidicBot.settings.timeGuard = !acidicBot.settings.timeGuard;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.timeguard}));
                        }
            ...
            toggleblCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.blacklistEnabled) {
                          return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.blacklist}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.blacklist}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.motdEnabled) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.motd}));
                        }
                        else {
                            acidicBot.settings.motdEnabled = !acidicBot.settings.motdEnabled;
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.motd}));
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.voteSkip) {
                            ...
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.voteskip}));
                        }
                        else {
                            acidicBot.settings.voteSkip = !acidicBot.settings.voteSkip;
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.voteskip}));
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        setTimeout(function (chat) {
                            ...
                            if (!found) {
                                $(".icon-chat").click();
                                return API.sendChat(subChat(acidicBot.chat.notbanned, {name: chat.un}));
                            }
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (permFrom > permUser) {
                            try {
                                ...
                                API.sendChat(subChat(acidicBot.chat.notmuted, {name: chat.un}));
                            }
                        }
                        else API.sendChat(subChat(acidicBot.chat.unmuterank, {name: chat.un}));
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(cd)) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.commandscd, {name: chat.un, time: acidicBot.settings.commandCooldown}));
                        }
                        else return API.sendChat(subChat(acidicBot.chat.invalidtime, {name: chat.un}));
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.usercommandsEnabled) {
                            API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.usercommands}));
                            acidicBot.settings.usercommandsEnabled = !acidicBot.settings.usercommandsEnabled;
                        }
                        else {
                            API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.usercommands}));
                            acidicBot.settings.usercommandsEnabled = !acidicBot.settings.usercommandsEnabled;
                        }
                    }
                }
            ...
            voteratioCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var name = msg.substring(cmd.length + 2);
                        var user = acidicBot.userUtilities.lookupUserName(name);
                        if (user === false) return API.sendChat(subChat(acidicBot.chat.invaliduserspecified, {name: chat.un}));
                        var vratio = user.votes;
                        var ratio = vratio.woot / vratio.meh;
                        API.sendChat(subChat(acidicBot.chat.voteratio, {name: chat.un, username: name, woot: vratio.woot, mehs: vratio.meh, ratio: ratio.toFixed(2)}));
                    }
                }
            },
            ...
            voteskipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length <= cmd.length + 1) return API.sendChat(subChat(acidicBot.chat.voteskiplimit, {name: chat.un, limit: acidicBot.settings.voteSkipLimit}));
                        var argument = msg.substring(cmd.length + 1);
                        if (!acidicBot.settings.voteSkip) acidicBot.settings.voteSkip = !acidicBot.settings.voteSkip;
                        if (isNaN(argument)) {
                            API.sendChat(subChat(acidicBot.chat.voteskipinvalidlimit, {name: chat.un}));
                        }
                        else {
                            acidicBot.settings.voteSkipLimit = argument;
                            API.sendChat(subChat(acidicBot.chat.voteskipsetlimit, {name: chat.un, limit: acidicBot.settings.voteSkipLimit}));
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (acidicBot.settings.welcome) {
                            ...
                            return API.sendChat(subChat(acidicBot.chat.toggleoff, {name: chat.un, 'function': acidicBot.chat.welcomemsg}));
                        }
                        else {
                            acidicBot.settings.welcome = !acidicBot.settings.welcome;
                            return API.sendChat(subChat(acidicBot.chat.toggleon, {name: chat.un, 'function': acidicBot.chat.welcomemsg}));
                        }
                    }
                }
            },
            ...
            websiteCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.website === "string")
                            API.sendChat(subChat(acidicBot.chat.website, {link: acidicBot.settings.website}));
                    }
                }
            },
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        for (var i = 0; i < len; ++i){
                            if (users[i].username == name){
                                ...
                                API.sendChat(subChat(acidicBot.chat.whois, {name1: chat.un, name2: name, id: id, avatar: avatar, profile: profile, language: language, level: level, status: status, joined: joined, rank: rank}));
                            }
                        }
                    }
                }
            ...
            youtubeCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!acidicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof acidicBot.settings.youtubeLink === "string")
                            API.sendChat(subChat(acidicBot.chat.youtube, {name: chat.un, link: acidicBot.settings.youtubeLink}));
                    }
                }
            }
        }
